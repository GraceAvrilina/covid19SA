...
@Component({
  selector: 'app-search-approval-result',
  templateUrl: './search-approval-result.component.html',
  styleUrls: ['./search-approval-result.component.scss']
})
export class SearchApprovalResultComponent implements OnInit {
  ...
  constructor(private globalService: GlobalService, private router: Router, private loadingCtrl: LoadingController) {
  ...
  getDataApproval(start = 0) {
    ...
    let tgl_kejadian = this.filter.tgl_kejadian.substr(0, 10)
    let tgl_kirim = this.filter.tgl_kirim.substr(0, 10)
    let params = {
      startPage: start,
      endPage: 10,
      status: this.type_segment,
      username: localStorage.getItem('username'),
      jenis: this.filter.jenis_laporan,
  ...
  getDataApprovalByStatus(start = 0, status, callback = null) {
    let helper = new Helper()
    this.loading_spinner = true
    let tgl_kejadian = this.filter.tgl_kejadian.substr(0, 10)
    let tgl_kirim = this.filter.tgl_kirim.substr(0, 10)
  ...
  close() {
    this.router.navigate(['home/approval-leader/search'])
  }
